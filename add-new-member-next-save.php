	...
	if($member_id > 0)
	{
				...
				$sql="update group_member set gm_password='".md5($pwd)."',gm_phone=".$mobileNo.",gm_image='".$uploaded_file."',status='Y' where gm_id=".$member_id;//echo $sql;exit();
								...
								if($tableResult > 0)
								{
										...
										$body.='<div align="left">Your Password is: '.$pwd.'</div>
										</div></div>';
				...
				<a href="http://'.$_SERVER['SERVER_NAME'].'/villageExpert/index.php?success=1&type=GM&email='.$Email.'" style="color:#fff;">Click to Login</a></p>';
